...
inline unsigned char *fast_rescaleImage(unsigned char *src, int src_x, int src_y, int dest_x, int dest_y) {
  /*make function inline*/
	
  /* move some variable declarations to most inner scope. */
	...
	double step_x=(double)(src_x-1)/(double)(dest_x-1), step_y=(double)(src_y-1)/(double)(dest_y-1); // Step increase as per instructions above
	
	int prexfl = -1, preyfl = -1, prexcl=-1, preycl = -1; // record previous floors/ceils
  ...
  /* first loop y to reduce cache misses*/
	for (y=0;y<dest_y;y++) 
  for (x=0;x<dest_x;x++) {
		double fx=x*step_x, fy=y*step_y;				// Corresponding coordinates on source image
    int flx= floor(fx), fly= floor(fy), clx=ceil(fx) , cly = ceil(fy);;
    double dx=fx-flx, dy=fy-fly;		// update dx dy
		if(flx != prexfl || fly != preyfl || clx != prexcl || cly != preycl) {
      /* if two floors are equal, current pixel has the same 4 corners as the previous one, don't need to call getPixel() again.*/
			prexfl = flx; preyfl = fly; prexcl=clx; preycl = cly;// update previous floors/ceils
    ...
    /*discard the use of some local variables and reduce number of operations by combining equations*/
    setPixel(dst,x,y,dest_x,
      (unsigned char)(dx*dy*(R4-R3-R2+R1) + dy*(R3-R1) + dx*(R2-R1) + R1),
      (unsigned char)(dx*dy*(G4-G3-G2+G1) + dy*(G3-G1) + dx*(G2-G1) + G1),
      (unsigned char)(dx*dy*(B4-B3-B2+B1) + dy*(B3-B1) + dx*(B2-B1) + B1));  
	}
	return(dst);
}
